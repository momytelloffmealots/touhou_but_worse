...
namespace _Project._Scripts.Gameplay.Projectiles
{
    ...
    public class Bullet : MonoBehaviour
    {
        ...
        private static Transform cachedEnemyTarget;
        ...
        private void FindHomingTarget()
        {
            if (isEnemyBullet)
            {
                if (cachedPlayerTarget == null || !cachedPlayerTarget.gameObject.activeInHierarchy)
                {
                    ...
                    if (target != null)
                    {
                        cachedPlayerTarget = target.transform;
                    }
                }
                homingTarget = cachedPlayerTarget;
            }
            else
            {
                if (cachedEnemyTarget == null || !cachedEnemyTarget.gameObject.activeInHierarchy)
                {
                    GameObject target = GameObject.FindGameObjectWithTag("Enemy");
                    if (target != null)
                    {
                        cachedEnemyTarget = target.transform;
                    }
                }
                homingTarget = cachedEnemyTarget;
            }
        }
        ...
        )
        {
                ...
                other.CompareTag("Enemy"))
            {
                ...
                // Nếu mục tiêu chưa có EnemyHealth thì gọi SendMessage dự phòng
